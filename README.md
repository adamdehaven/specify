...
> **NOTE**: If you receive an error that the plugin is not compatible with the installed version of your version of Adobe Illustrator, or if the plugin becomes incompatible after an Adobe update, navigate to "Manage Plugins" within the Creative Cloud desktop application. Uninstall the Specify plugin, and then simply re-install the plugin.
